...
const OfferPopup = () => {
  ...
  // Rate limiting for offer claims
  const {
    isLimited,
    checkRateLimit,
    recordAttempt
  } = useRateLimit(5, 300000); // 5 claims per 5 minutes
  ...
  const handleClaim = () => {
    // Check rate limit before processing claim
    if (!checkRateLimit()) {
      alert('Too many offer claims. Please wait a few minutes before trying again.');
      return;
    }
    ...
    if (currentOffer.promoCode) {
      ...
      navigator.clipboard.writeText(currentOffer.promoCode).then(() => {
        alert(`Promo code "${currentOffer.promoCode}" copied to clipboard!`);
      }).catch(() => {
        // Fallback if clipboard API is not available
        alert(`Your promo code is: ${currentOffer.promoCode}`);
      });
    }
            ...
            <div className="rounded-2xl overflow-hidden shadow-2xl">
              ...
              <div className="relative p-6 text-center">
                ...
                <button
                  ...
                  style={{
                    backgroundColor: isLimited ? '#9CA3AF' : (currentOffer.buttonColor || '#f97316'),
                    color: '#ffffff'
                  }}
                >
                  {isLimited ? 'Please Wait...' : (currentOffer.buttonText || 'Claim Offer')}
                </button>
